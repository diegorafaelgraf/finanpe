...
cuentaCtrl.agregarCuenta = async (req, res) => {
    const { tipo_cuenta, moneda, saldo } = req.body;
    const errors = [];
    if(tipo_cuenta == ''){
    ...
    if(errors.length > 0){
        ...
        res.render('cuenta/cuenta_add', {errors, tipocuenta_todas, moneda_todas, saldo});       
    }
    else{
        const cuenta = await Cuenta.findOne({moneda: moneda, tipo_cuenta: tipo_cuenta});
        if(cuenta){
            errors.push({text: 'La cuenta ingresada ya existe'});
            res.render('cuenta/cuenta_add', {errors, tipo_cuenta, moneda});
        }
        else{
            const cuenta = new Cuenta({moneda: moneda, tipo_cuenta: tipo_cuenta, saldo: saldo})
            try{
                await cuenta.save();
